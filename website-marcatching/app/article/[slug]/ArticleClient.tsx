...
function ArticleBlockRenderer({ block, products, onOpenProduct }: { block: ArticleBlock; products: Product[]; onOpenProduct: (p: Product) => void }) {
  if (block.type === 'headline') {
    return <div className={styles.blockHeadline} style={{ fontSize: HEADLINE_SIZES[block.size] || '1.5rem', color: block.color || '#ffffff', textAlign: block.align as any || 'left', fontFamily: block.font_family || 'DM Sans, sans-serif' }}>{block.text}</div>
  }
  if (block.type === 'text') {
    return (
      <div className={styles.blockText} style={{ fontSize: block.size || '1rem', fontWeight: WEIGHT_MAP[block.weight || 'normal'], fontStyle: block.italic ? 'italic' : 'normal', color: block.color || 'rgba(255,255,255,0.85)', textAlign: block.align as any || 'left', fontFamily: block.font_family || 'DM Sans, sans-serif' }}>
        {block.text.split('\n').map((line, i) => <span key={i}>{line}{i < block.text.split('\n').length - 1 && <br />}</span>)}
      </div>
    )
  }
  if (block.type === 'image') {
