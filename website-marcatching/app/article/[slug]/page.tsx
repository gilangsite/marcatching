...
import type { Article, ArticleBlock, Product } from '@/lib/supabaseClient'
  ...
  if (block.type === 'headline') {
    return (
      <div
        ...
        style={{
          fontSize: HEADLINE_SIZES[block.size] || '1.5rem',
          color: block.color || '#ffffff',
          textAlign: block.align as any || 'left',
          fontFamily: block.font_family || 'DM Sans, sans-serif',
        }}
      >
        {block.text}
      </div>
  ...
  if (block.type === 'text') {
    return (
      <div
        ...
        style={{
          fontSize: block.size || '1rem',
          fontWeight: WEIGHT_MAP[block.weight || 'normal'],
          fontStyle: block.italic ? 'italic' : 'normal',
          color: block.color || 'rgba(255,255,255,0.85)',
          textAlign: block.align as any || 'left',
          fontFamily: block.font_family || 'DM Sans, sans-serif',
        }}
      >
        {block.text.split('\n').map((line, i) => (
          <span key={i}>{line}{i < block.text.split('\n').length - 1 && <br />}</span>
        ))}
      </div>
    )
  }
      ...
      <div className={styles.blockVideo}>
        <div className={styles.blockVideoWrap}>
          <iframe
            ...
            allowFullScreen
            className={styles.blockVideoFrame}
          />
        </div>
        {block.caption && <p className={styles.blockImageCaption}>{block.caption}</p>}
      </div>
        ...
        <div className={styles.blockProductInner}>
          {thumb && (
            <div className={styles.blockProductThumb}>
              ...
              {product.discount_percentage > 0 && (
                <span className={styles.blockProductBadge}>-{product.discount_percentage}%</span>
              )}
            </div>
          )}
          ...
          <div className={styles.blockProductCta}>
        ...
        <main className={styles.main}>
          <div className={styles.loadingWrap}>
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className={styles.skeleton} style={{ height: i === 0 ? 48 : i === 1 ? 24 : 120, width: i === 1 ? '60%' : '100%' }} />
            ))}
          </div>
        </main>
      </>
      ...
      <>
        ...
        <main className={styles.main}>
          <div className={styles.notFound}>
            ...
            <Link href="/article" className={styles.backLink}><ArrowLeft size={16} /> Kembali ke Articles</Link>
          </div>
        </main>
      </>
    )
    ...
    <>
      ...
      <main className={styles.main}>
        <article className={styles.article}>
          ...
          {/* Category badge */}
          {category && (
            <div className={styles.articleCat}>
          ...
          <div className={styles.articleMeta}>
            ...
            {!author && (
              <div className={styles.articleMetaSimple}>
          ...
          {author && (
            <div className={styles.authorFooter}>
              ...
              <div>
                <div className={styles.authorFooterLabel}>Ditulis oleh</div>
                <div className={styles.authorFooterName}>{author.name}</div>
              </div>
            </div>
          )}
          ...
          <div className={styles.articleEnd}>
            <Link href="/article" className={styles.backBtnLg}>
              <ArrowLeft size={16} /> Kembali ke Articles
            </Link>
          </div>
        </article>
      </main>
