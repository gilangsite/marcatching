...
## Marcatching Skill Main Integration Rule

Prompt library ini sudah disesuaikan dengan **Marcatching Skill Main v1.1**.

Di arsitektur baru, prompt tidak lagi dijalankan bersama dua dokumen lama seperti dua dokumen skill lama.

Sekarang user cukup menjalankan satu skill utama:

```text
/marcatching-skill-main
```

Marcatching Skill Main adalah satu installable skill dengan satu `SKILL.md`. File pendukung seperti marketing intelligence, content creation, research safety, brand memory, evaluator, dan prompt libraries berada di folder `resources/` dan akan dibaca sesuai kebutuhan.

Urutan penggunaan yang benar:

```text
1. Aktifkan atau jalankan Marcatching Skill Main.
2. Copy prompt dari library ini.
3. Isi field konteks dengan data brand, produk, audiens, channel, objective, proof, dan constraint.
4. Jalankan prompt.
5. Skill Main akan melakukan diagnosis, memilih resource yang relevan, mengevaluasi output, dan melakukan rewrite jika output masih lemah.
```

Untuk marketing dan copywriting, Skill Main wajib menjaga:
...
Jika brand/product context masih tipis, Skill Main harus menjalankan Brand Learning Gate atau meminta user memberikan website, product page, social media, screenshot, deck, atau dokumen brand.

Jika prompt memakai data, klaim, tren, statistik, medical, legal, financial, atau fakta yang berisiko, Skill Main harus menjalankan Research Safety Gate sebelum membuat final output.

Jangan meminta user attach dua dokumen lama. Jangan meminta user attach 11 modular files satu per satu. Gunakan **Marcatching Skill Main v1.1** sebagai operating system tunggal.
...
- Fixed legacy wording from previous two-document setup.
...
### Version 1.2.0
Adjusted:
- Updated integration from old two-document setup to single `Marcatching Skill Main v1.1`.
- Removed dependency wording that asked users to load the old two-document setup.
- Added Brand Learning Gate for thin brand/product context.
- Added Research Safety Gate for factual, medical, legal, financial, statistical, or trend-based claims.
- Added stronger evaluator requirement: rewrite if important score is below 8/10.
- Updated every Full Prompt with Skill Main execution rule.

### Version 1.1.0
Added:
- Required instruction for AI to read the main Marcatching operating skill before executing each prompt.
- Required instruction for AI to use the main Marcatching copy and evaluation system before finalizing output.
